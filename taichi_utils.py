...
def sizing_function(u: ti.template(), sizing: ti.template(), dx: float):
    ...
    for i, j, k in u:
        ...
        partial_x = 1.0 / (2 * dx) * (u_r - u_l)
        partial_y = 1.0 / (2 * dx) * (u_t - u_b)
        partial_z = 1.0 / (2 * dx) * (u_c - u_a)
        if i == 0:
            partial_x = 1.0 / (2 * dx) * (u_r - 0)
        elif i == u_dim - 1:
            partial_x = 1.0 / (2 * dx) * (0 - u_l)
        if j == 0:
            partial_y = 1.0 / (2 * dx) * (u_t - 0)
        elif j == v_dim - 1:
            partial_y = 1.0 / (2 * dx) * (0 - u_b)
        if k == 0:
            partial_z = 1.0 / (2 * dx) * (u_c - 0)
        elif k == w_dim - 1:
            partial_z = 1.0 / (2 * dx) * (0 - u_a)

        sizing[i, j, k] = ti.sqrt(
            partial_x.x**2
            + partial_x.y**2
            + partial_x.z**2
            + partial_y.x**2
...
def diffuse_grid(value: ti.template(), tmp: ti.template()):
    ...
    for i, j, k in tmp:
        tmp[i, j, k] = (
            ...
            / 6
            * (
                sample(value, i + 1, j, k)
                + sample(value, i - 1, j, k)
                + sample(value, i, j + 1, k)
                + sample(value, i, j - 1, k)
                + sample(value, i, j, k + 1)
                + sample(value, i, j, k - 1)
            )
        )
...
def interp_grad_1(vf, p, dx, BL_x=0.5, BL_y=0.5, BL_z=0.5):
    ...
    v = v - BL_y
    w = w - BL_z
    s = ti.max(1.0, ti.min(u, u_dim - 1 - eps))
    t = ti.max(1.0, ti.min(v, v_dim - 1 - eps))
    l = ti.max(1.0, ti.min(w, w_dim - 1 - eps))
    ...
    for i in range(0, 2):
        for j in range(0, 2):
            for k in range(0, 2):
                ...
                partial_x += 1.0 / dx * (value * dN_1(x_p_x_i) * N_1(y_p_y_i) * N_1(z_p_z_i))
                partial_y += 1.0 / dx * (value * N_1(x_p_x_i) * dN_1(y_p_y_i) * N_1(z_p_z_i))
                partial_z += 1.0 / dx * (value * N_1(x_p_x_i) * N_1(y_p_y_i) * dN_1(z_p_z_i))
...
def interp_1(vf, p, dx, BL_x=0.5, BL_y=0.5, BL_z=0.5):
    ...
    interped = 0.0 * sample(vf, iu, iv, iw)
    ...
    for i in range(-1, 3):
        for j in range(-1, 3):
            for k in range(-1, 3):
                ...
                z_p_z_i = l - (iw + k)
                value = sample(vf, iu + i, iv + j, iw + k)
                partial_x += 1.0 / dx * (value * dN_2(x_p_x_i) * N_2(y_p_y_i) * N_2(z_p_z_i))
                partial_y += 1.0 / dx * (value * N_2(x_p_x_i) * dN_2(y_p_y_i) * N_2(z_p_z_i))
                partial_z += 1.0 / dx * (value * N_2(x_p_x_i) * N_2(y_p_y_i) * dN_2(z_p_z_i))
