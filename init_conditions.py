...
@ti.kernel
def add_vortex_ring_and_smoke(
    w: float,
    rad: float,
    delta: float,
    c: ti.types.vector(3, float),
    unit_x: ti.types.vector(3, float),
    unit_y: ti.types.vector(3, float),
    pf: ti.template(),
    vf: ti.template(),
    smokef: ti.template(),
    ...
    for i, j, k in vf:
        for l in range(num_samples):
            ...
            vf[i, j, k] += (
                curve_length * (-1 / (4 * math.pi * r**3) * (1 - ti.exp(-((r / delta) ** 3)))) * p_diff.cross(w_vector)
            )
    ...
    for i, j, k in smokef:
        if smokef[i, j, k][3] > 0.002:
            ...
            smokef[i, j, k] = ti.Vector([0.0, 0.0, 0.0, 0.0])
...
def init_four_vorts(X, u, smoke1, smoke2):
    ...
    add_vortex_ring_and_smoke(
        w=2.0e-2,
        rad=size,
        delta=0.024,
        c=ti.Vector([0.5 - x_offset, 0.5 - y_offset, 1]),
        unit_x=ti.Vector([-cos45, cos45, 0.0]).normalized(),
        unit_y=ti.Vector([0.0, 0.0, 1.0]),
        pf=X,
        vf=u,
        smokef=smoke1,
        color=ti.Vector([1, 0.8, 0.7]),
        num_samples=500,
    )
