...
class NeuralBuffer3:
    ...
    def set_magnitude_scale(self, max_speed):
        self.magnitude_scale = 1.0 / max_speed
    ...
    def sample(self, curr_u_x, curr_u_y, curr_u_z, init_u_x, init_u_y, init_u_z, mid_ts, importance=None):
        ...
        coord_x_sampled = self.dx * torch.cat(
            (x_sampled[:, [0]], x_sampled[:, [1]] + 0.5, x_sampled[:, [2]] + 0.5), dim=-1
        )
        ...
        coord_y_sampled = self.dx * torch.cat(
            (y_sampled[:, [0]] + 0.5, y_sampled[:, [1]], y_sampled[:, [2]] + 0.5), dim=-1
        )
        ...
        coord_z_sampled = self.dx * torch.cat(
            (z_sampled[:, [0]] + 0.5, z_sampled[:, [1]] + 0.5, z_sampled[:, [2]]), dim=-1
        )
        curr_coord_zt_sampled = torch.cat((coord_z_sampled, curr_coord_t), dim=-1)[:, None, :]

        curr_coord_sampled = torch.cat(
            (curr_coord_xt_sampled, curr_coord_yt_sampled, curr_coord_zt_sampled), dim=1
        )  # [N, 3, 4]
        ...
        if n_frames > 1:
            ...
            coord_x_sampled = (
                self.dx
                * torch.cat((xt_sampled[:, [0]], (xt_sampled[:, [1]] + 0.5), xt_sampled[:, [2]] + 0.5), dim=-1)[
                    :, None, :
                ]
            )  # [N, 1, 3]
            coord_y_sampled = (
                self.dx
                * torch.cat(((yt_sampled[:, [0]] + 0.5), yt_sampled[:, [1]], yt_sampled[:, [2]] + 0.5), dim=-1)[
                    :, None, :
                ]
            )  # [N, 1, 3]
            coord_z_sampled = (
                self.dx
                * torch.cat(((zt_sampled[:, [0]] + 0.5), zt_sampled[:, [1]] + 0.5, zt_sampled[:, [2]]), dim=-1)[
                    :, None, :
                ]
            )  # [N, 1, 3]
            ...
            prev_u_sampled = self.model_prev(
                torch.cat((coord_xyz_sampled, self.prev_t_ratio * t_sampled), dim=-1)
            )  # [N, 3, 4]
    ...
    def store_u(self, curr_u_x, curr_u_y, curr_u_z, init_u_x, init_u_y, init_u_z, dts):
        # rescale
        ...
        if (self.prev_t_ratio is not None) and self.prev_t_ratio / self.curr_t_ratio > 1.33:
            ...
            print(
                "[Neural Buffer] Re-randomizing weights since ratio difference is: ",
                (self.prev_t_ratio / self.curr_t_ratio).cpu().numpy(),
            )

        optimizer = AdamW([{"params": self.model.parameters()}], lr=self.lr, betas=(0.9, 0.99))
        ...
        for i in range(self.N_iters):
            ...
            with torch.no_grad():
                coord_sampled, u_sampled = self.sample(
                    curr_u_x, curr_u_y, curr_u_z, init_u_x, init_u_y, init_u_z, self.mid_ts, importance
                )  # sample from current time
    ...
    def pred_u_x(self, coord, mid_t):
        ...
        return 1.0 / self.magnitude_scale * self.model.eval_x(coord_xyzt)

    # coord [N, 3]
    # return [N, 1]
    def pred_u_y(self, coord, mid_t):
        coord_xyzt = torch.cat((coord, self.curr_t_ratio * mid_t * torch.ones(coord.shape[0], 1)), axis=-1)
        return 1.0 / self.magnitude_scale * self.model.eval_y(coord_xyzt)

    # coord [N, 3]
    # return [N, 1]
    def pred_u_z(self, coord, mid_t):
        coord_xyzt = torch.cat((coord, self.curr_t_ratio * mid_t * torch.ones(coord.shape[0], 1)), axis=-1)
        return 1.0 / self.magnitude_scale * self.model.eval_z(coord_xyzt)
